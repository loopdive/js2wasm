...
When starting work:
1. Check `plan/method/file-locks.md` for conflicts
...
On completion: remove claim from `plan/method/file-locks.md`
...
The `files` field is a **planned lock claim**. At runtime, active locks are tracked in `plan/method/file-locks.md`. Before starting, devs check the lock table. Overlap requires developer-to-developer coordination via messages or PO approval.
...
4. Add entry to `plan/log/issues-log.md`
5. Check blocked issues and update their `status` in frontmatter when they become actionable
6. Update `plan/log/dependency-graph.md`
