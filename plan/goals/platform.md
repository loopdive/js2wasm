...
| **639** | Full Component Model adapter (canonical ABI) |  | ready | critical |
| **640** | WASI HTTP handler (unlocks serverless edge) |  | ready | high |
| **641** | Shopify Functions template |  | ready | medium |
| **642** | Deno/Cloudflare loader plugins |  | ready | low |
...
| **807** | Extract Date/Math/console built-ins from expressions.ts → builtins.ts |  | ready | medium |
| **837** | Stage 3: Map/WeakMap upsert — getOrInsert, getOrInsertComputed |  | ready | low |
| **863** | decodeURI/encodeURI failures: #0-0 error pattern (70 FAIL) | 35 | done | low |
| **865** | Console wrapper for fd_write in JavaScript environments |  | ready | medium |
...
| **1029** | Migrate to TypeScript 7.x (Go rewrite / typescript-go) when compiler API stabilizes |  | backlog | low |
| **1045** | DOM globals as extern classes (DOM_HOST_GLOBALS, queueMicrotask, requestAnimationFrame) | 45 | ready | high |
| **1066** | Support eval in standalone mode via host-compiled Wasm child module |  | ready | medium |
...
| **1136** | Reference platform scenario: capability-safe DOM wrapper with explicit subtree authority |  | ready | high |
| **1137** | Reference platform scenario: run a Node-oriented example on Wasmtime via Edge.js |  | ready | high |
| **1165** | Track Wasm JIT interface proposal (func.new) — native runtime codegen |  | ready | medium |
