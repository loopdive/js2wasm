...
After #1231 registers Array as a pseudo-`ExternClassInfo`, the
...
- Callback-taking methods: `.map`, `.filter`, `.reduce`, `.forEach`,
  ...
  IR's closure/function-reference model in ways not yet supported. A
  separate slice (#1234?) integrates IR-claimed callbacks into these
  methods.

## Dependency note

`depends_on: [1231]` was written when #1231's scope included Array
pseudo-`ExternClassInfo` registration. The shipped #1231 (struct field type
inference, Phase 1, propagator extension) does **not** register Array as an
ExternClassInfo. Before scheduling this issue, file a new spec for the
ExternClassInfo registration step and list that as the actual dependency.

## MLIR alignment

Per-element-type method variants must be expressed as `TypeMap` specialisations,
not hardcoded per-type dispatch in the emitter. The registration step synthesises
