...
# #1246 — `typeof x === "string"` guard breaks `String.prototype.substring(start)` (single-arg form)
...
A regression test in `tests/issue-1246.test.ts` should exercise both
...
2. `tests/issue-1246.test.ts` covers the single-arg `substring` form
