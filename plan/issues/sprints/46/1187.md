...
4. Pick one of: (a) trace every `ensureLateImport` reachable from `ensureNativeStringHelpers` and pre-register them all up front; (b) add a module-level shift flush; (c) abandon the Wasm-side helper approach and expose JS-side wrappers via `buildImports.testHelpers`.
