...
Same root cause as #991: deep recursive AST walking in `bodyUsesArguments()`,
fixed by #1085 / PR #127 with an iterative walker. Closing with no additional
code change.

Verified on main (HEAD `6e8835fc`) via `.tmp/verify-993.mts` — compile-only,
no execution needed:

| Test | Compile time |
|------|--------------|
| `S12.14_A9_T3.js` | ~430ms |
| `S12.14_A11_T3.js` | ~210ms |
| `S12.14_A12_T3.js` | ~180ms |

Max 430ms — two orders of magnitude below the 30s worker timeout.

The same resolution was already recorded on branch `issue-993-try-timeout`
(commit `ab154c48`) but hasn't reached `main` yet.
