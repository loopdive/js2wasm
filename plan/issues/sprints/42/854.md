...
## Test Results (sprint 42 — PR #144)

Branch `issue-854-iterator-null-methods` (HEAD: 5145058e — merge of main into branch) covers:

1. `src/codegen/array-methods.ts`: add `@@iterator` to `ARRAY_METHODS`, route to `compileArrayIteratorMethod` with `"values"` per ES spec; switch array-iterator imports to `ensureLateImport` so function-index shifting fires correctly.
2. `src/codegen/property-access.ts`: in `compileElementAccessBody`, when the computed key resolves to a well-known symbol (`@@…`), route through `extern.convert_any` + `__extern_get` so the runtime can look it up on WasmGC vec structs.
3. `src/runtime.ts`: move `__extern_get` into the `case "extern_get"` handler and add a branch that returns a values-iterator factory for `_isWasmStruct(obj) && key === Symbol.iterator` (falls through to `undefined` when `__vec_len`/`__vec_get` exports are unavailable, so no regression on non-vec structs).
4. `tests/issue-854-smoke.test.ts`: 8/10 pass. Pass list includes `arr.entries()`/`.keys()`/`.values()` for-of, basic for-of, and the test262 `Array.prototype.keys` case. The 2 failures are edge cases (`arr[Symbol.iterator]` accessed as a property without a subsequent call) where `__vec_len`/`__vec_get` exports are not emitted — intentionally narrow to avoid the 1545 `range_error` regression that tripped commit `b9159888`.

Local sample of the test262 cases named in the issue:
- `Array.prototype.keys.js` — PASS
- `Array.prototype.entries.js`, `Array.prototype.Symbol.iterator.js` — now compile-error due to main commit `d8cfbb7a` (strict type-annotation mismatch on heterogeneous array literals), unrelated to iterator protocol work.
- `generic-iterable.js` — still fails: `iterator.next is not a function`. Generic JS objects with user-assigned `[Symbol.iterator]` are not fully handled by the runtime `__iterator` path for WasmGC-struct iterator results; needs a follow-up to route `.next()` calls on struct iterators through a sidecar/`__call_next` export.

CI: GitHub Actions for PR #144 cancelled at billing/spending limit — no test262 shard ran, so the 126-test impact cannot be measured from CI yet. Re-run once the limit is lifted.
