...
## Resolution (sprint 42)

Root cause was deep recursive AST walking in `bodyUsesArguments()`, not
anything iterator-helper-specific. Fix landed in #1085 / PR #127 by converting
the traversal to an iterative walker. Closing this issue with no additional
code change.

Verified on main (HEAD `6e8835fc`) via `.tmp/verify-991.mts` — compile-only,
no execution needed:

| Test | Compile time |
|------|--------------|
| `filter/throws-typeerror-when-generator-is-running.js` | ~400ms |
| `flatMap/throws-typeerror-when-generator-is-running.js` | ~200ms |
| `map/throws-typeerror-when-generator-is-running.js` | ~160ms |

Max 400ms — well under the 5s acceptance threshold and two orders of magnitude
below the 30s worker timeout these tests used to burn.

The same resolution was already recorded on branch `issue-993-try-timeout`
(commit `632560e5`) alongside #995 and #996, but hasn't reached `main` yet.
