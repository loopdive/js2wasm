...
### Case 1: Module-init-only (no exports, no main) — the issue example
- `__init_done` no longer emitted: PASS
- `_start` exported directly: PASS
- `_start()` executes correctly (prints 1000000): PASS
- Binary size reduced: 278 → 266 bytes

### Case 2: Exported function + top-level init (guard still needed)
- `__init_done` guard preserved: PASS
- `test()` returns correct values (1, 2): PASS

### Case 3: Has `main()` — init prepended (unchanged)
- `main()` returns 42: PASS

### Case 4: test262 pattern (export + init)
- Guard triggers on first `test()` call: PASS

## Implementation

Modified `src/codegen/declarations.ts` (line ~2514): when there are no user-exported
functions and no `main()`, the compiler now exports `__module_init` as `_start` directly
instead of creating an `__init_done` guard global. The guard path is preserved for
programs that have exports but no `main()` (lazy init on first export call).

Updated `playground/main.ts` to call `_start()` for module-init-only programs instead
of synthesizing a `main()` function via recompilation.
