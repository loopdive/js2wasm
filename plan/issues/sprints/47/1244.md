...
- #1246 — `typeof x === "string"` guard breaks `substring(start)` (NEW, surfaced by Tier 1c)
- #1247 — typed `string[]` local with `path.split("/")` initializer triggers struct-type mismatch (NEW, surfaced by Tier 1a)

## Tier 1 result (2026-05-02)

`tests/stress/hono-tier1.test.ts` — **3 of 3 Tier 1 tests pass**, Tier 2
documented as `it.skip` with the gap-blocking issue references:

| tier | test | result |
|---|---|---|
| 1a | `splitPath` (5 cases: basic, root, no-leading-slash, trailing-slash, deep) | ✓ pass |
| 1b | static path equality match (3 cases: match, no-match-prefix, no-match-different) | ✓ pass |
| 1c | parameterized path match (5 cases: static, one-param, two-params, no-match-arity, no-match-static-diff) | ✓ pass |
| 2  | `compileProject` on `hono/dist/router/trie-router/router.js` end-to-end | skip (gap) |

**New compiler gaps surfaced**:
- **#1246** — `typeof x === "string"` guard breaks `substring(start)`. The Tier 1c parameterised-route matcher hit this on the very first attempt; workaround was to drop the typeof guard.
- **#1247** — typed `string[]` local with `path.split("/")` initializer fails Wasm validation with a struct-type mismatch (`(ref null 6)` vs `(ref null 1)`). The Tier 1a `splitPath` signature had to be rewritten with `any` typing.

**Tier 2 blockers** (not in scope for this PR):
- Class private fields (`#methods`, `#children`, `#patterns`, `#order`, `#params`) — `TrieRouter` and `Node` use them throughout
- `Object.create(null)` — used as the children/methods/params raw maps
- Spread-into-existing-object `{ ...node.#params }` — used during pattern match
- `||=` (logical OR assignment), optional chaining (`m[0].match(/\//)?.length ?? 0`)
- Generic `Array<T>.indexOf(v, i, a)` callback signatures

These need a dedicated set of issues; #1244's stretch scope was Tier 1, which is now landing.
