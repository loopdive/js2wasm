...
- #1248 — `typeof x === "string"` guard breaks `substring(start)` (NEW, surfaced by Tier 1c)
...
- **#1248** — `typeof x === "string"` guard breaks `substring(start)`. The Tier 1c parameterised-route matcher hit this on the very first attempt; workaround was to drop the typeof guard.
