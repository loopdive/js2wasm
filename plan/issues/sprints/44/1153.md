...
## Test Results (2026-04-23, descriptor-snapshot sandbox rewrite)

**Sandbox smoke probe** (`.tmp/probe-1153-sandbox.mjs`, 13 scenarios):

- delete `Array.prototype.reduce`, `WeakMap.prototype.set`, `RegExp.prototype.exec` (the three documented #1153 crash triggers) — all restored, preserved `enumerable:false`, function identity matches originals.
- replace `Array.prototype.reduce` value (hot-path IC case) — restored via assignment, identity preserved.
- poison `RegExp.prototype.flags` accessor (#1157) — restored via `defineProperty`.
- `Object.prototype` key pollution — deleted.
- `Array.prototype[0]` numeric poisoning — deleted.
- delete `Array.from` static method — restored.
- poison `Array.prototype[Symbol.iterator]` — restored (the restore loop itself avoids `for...of`/destructuring so it can't be tripped by a poisoned iterator).
- delete `Map.set` / `Set.add` / `Promise.then` — all restored.
- No-op restore when pristine — preserves function identity (doesn't disturb V8 ICs).
- Compound poisoning (Symbol.iterator + reduce + WeakMap.set + RegExp.exec in one pass) — all restored together.
- Poison `Object.getOwnPropertyDescriptor` itself — restore still works because we stash native refs at module load.

**Integration probe** (`.tmp/probe-1153-integration.mjs`, spawns real worker, 8 scenarios):
With the fix: every poison pattern (steps 2-7) is followed by a compile that does NOT return any of the four #1153 crash signatures (`constructSigs.reduce`, `cache.set`, `commentDirectiveRegEx.exec`, `shebangTriviaRegex.exec`). Back-to-back compile consistency confirmed.
Negative control (same probe, `restoreBuiltins()` commented out): Symbol.iterator poisoning and flags-accessor poisoning propagate into subsequent compiles and fail the probe (TS compiler throws "extensions is not iterable"). Demonstrates the restore call is load-bearing.

**Equivalence tests**: unchanged — worker file is not imported by any `tests/equivalence/` test, so this change has zero blast radius on the equivalence suite. Verified by running one failing equivalence test (`yield-as-expression`) on `main` before and after the fix: identical output, identical failure signature.

## Implementation Summary

Rewrote `scripts/test262-worker.mjs` lines 39-280 (replaced the `_METHOD_SNAPSHOTS` / `_STATIC_SNAPSHOTS` / `_ACCESSOR_SNAPSHOTS` whitelist and the three-section `restoreBuiltins`) with a single comprehensive descriptor-snapshot approach:

1. **Targets** (`_PROTOTYPE_TARGETS` + `_STATIC_TARGETS`): 33 compiler-critical objects (15 prototypes + 18 constructors/namespaces).
2. **Snapshot at startup**: `_captureSnapshot(obj)` enumerates `Reflect.ownKeys(obj)` and stores every `[key, descriptor]` in a plain array — no dependence on Map/Set iteration.
3. **Restore** (`_diffSnapshot`) runs two passes per target:
   - Pass 1 deletes any current own property not present at startup (poison additions).
   - Pass 2 restores every snapshot entry that is missing or drifted. Data-property value drift uses assignment (preserves V8 method ICs, the key hot-path optimization from #1153 attempt 1); deletions and accessor drift use `defineProperty` (preserves `enumerable:false` etc.).
4. **Non-configurable poison**: if a property is non-configurable and can't be restored, the worker logs `FATAL: non-configurable poison on <prototype>[<key>] — restarting worker` and exits, matching the existing pool-restart contract.
5. **Iteration-safety**: the restore logic avoids all iteration protocols (no `for...of`, no destructuring, no spread) and stashes `Reflect.ownKeys`, `Object.getOwnPropertyDescriptor`, `Object.defineProperty`, `Object.is` at module load. Smoke probe scenario 9 deliberately poisons `Array.prototype[Symbol.iterator]` before calling `restoreBuiltins()` to prove the restore doesn't itself trip over the poison.
