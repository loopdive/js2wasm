...
517 tests fail with "not enough arguments on the stack for struct.new". The compiler emits a `struct.new` instruction but pushes fewer values than the struct type requires.
