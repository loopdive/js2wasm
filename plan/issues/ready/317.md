...
- [ ] Add `usesAnyType` flag to `ModuleContext`, set during IR lowering when `any` is encountered
- [ ] Gate `$AnyValue` emission on `moduleContext.usesAnyType`
- [ ] Decide on canonical export emission strategy (inline vs. trailing)
- [ ] Add `emittedExports` deduplication guard
- [ ] Add regression test: compile a no-`any` module and assert `$AnyValue` is absent
- [ ] Add regression test: assert export section contains no duplicate names
