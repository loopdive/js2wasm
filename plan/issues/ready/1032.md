...
- **No Node host-import routing at all.** No `NODE_HOST_IMPORT_MODULES` set exists in the compiler; `import http from 'node:http'` hits `preprocessImports` at src/import-resolver.ts:23 and gets rewritten to `declare const http: any`, losing all symbol-level typing. This is the core issue of #1032 — needs a new compile-time hook in the import resolver that recognizes Node-builtin module specifiers and emits externref imports per used symbol instead.
- **`await` is a no-op** at src/codegen/expressions.ts:790. `AwaitExpression` recurses into its operand and returns unchanged — there is no Promise integration, no microtask suspension, no state-machine lowering. Any code path that exercises real I/O completion observes synchronous resolution only. Tracked as #1042.
...
- **`process.env` reads** currently fall through `preprocessImports` as a `declare const process: any`. Axios reads `process.env.NODE_ENV` and `process.env.HTTP_PROXY` at startup — works incidentally but not with correct types.
...
1. **`NODE_HOST_IMPORT_MODULES` compile-time routing** — the core feature this issue proposes. No existing code. Medium effort, ~1 sprint.
2. **Real `async`/`await` state-machine lowering** — tracked as **#1042**. Research-level; depends on #680 (Wasm-native generators). Without this, Tier 4 smoke test (real GET) cannot succeed.
...
**Nothing landed yet.** #1035 is the closest parallel work (WASI `node:fs`) but it routes through compile-time syscall translation, not externref host imports — those are two different mechanisms with overlapping surface. Recommend designing the Node-host-import path (this issue, #1032) and the WASI path (#1035) together so they share the module-specifier-recognition hook in the import resolver.

### Implementation sketch

```ts
// src/import-resolver.ts — new branch inside preprocessImports
...
// In the import rewriting loop, when isNodeBuiltin(spec):
//   emit `declare namespace <name> { function request(...): any; ... }` stubs
//   based on usage analysis (same machinery as other namespace imports)
//   AND mark the module name for late externref-import emission during codegen.
```
...
**Recommendation:** defer attempting axios until **both #1041 (pre-bundle) and the Node-host-import scaffold** are in place. Without `await` state-machine lowering (#1042), the Tier 4 "real GET from httpbin.org" stretch goal is not achievable — but Tiers 1-3 compile-cleanly is still a valid win.
