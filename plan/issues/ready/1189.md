...
# #1189 — Escape-analysis scalarization for non-escaping arrays
...
**Option B (preferred — large dense):** keep the array but allocate it once, sized to `n` upfront, and propagate this knowledge to bounds-check elimination (#1190) and i32 specialization (#1191).
...
The two compose multiplicatively. Phase 3 should be a separate issue (file as #1192 when this lands and Phase 1+2 are stable).
