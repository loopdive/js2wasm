...
## Step 8 complete: property-access.ts extracted

Extracted ~1,600 lines into `src/codegen/property-access.ts`:
- `compilePropertyAccess`, `compileElementAccess`, `compileElementAccessBody`
- `compileOptionalPropertyAccess`, `compileExternPropertyGet`, `compileExternPropertyGetFromStack`
- `typeErrorThrowInstrs`, `emitNullCheckThrow`, `findAlternateStructsForField`
- `emitNullGuardedStructGet`, `emitExternrefToStructGet`
- `emitBoundsCheckedArrayGet`, `emitClampIndex`, `emitClampNonNeg`
- `emitBoundsGuardedArraySet`, `isSafeBoundsEliminated`

Also created `src/codegen/shared.ts` for cross-module utilities:
- `VOID_RESULT`, `InnerResult`, `valTypesMatch`, `getLine`, `getCol`
- `compileExpression` delegate (registered at import time by expressions.ts)

Tests: 1,059 pass / 38 fail (unchanged).
