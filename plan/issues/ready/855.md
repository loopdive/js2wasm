...
**Key fixes:**
- `instanceof Promise` works (Promise objects stay as externref, not unwrapped to f64)
- `typeof Promise.all/race/etc === 'function'` works
- `Promise.allSettled` and `Promise.any` supported
- `.finally()` instance method supported
- `.then(onFulfilled, onRejected)` two-callback form works
- Promise combinator args: WasmGC arrays converted to JS arrays
- `.then()` callbacks fire via microtask queue (callback bridge already existed)
- Test runner drains microtasks for async tests via `__asyncResult` export

**Remaining 461 failures:** mostly tests requiring complex async chaining with mutable
captures across multiple `.then()` callbacks, Promise subclassing, or `Promise()` without
`new` throwing TypeError. These are fundamental async protocol issues beyond the scope of
this fix.
