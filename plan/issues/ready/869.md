...
1. At call sites with missing args: emit the default value directly when it's a constant
2. For expression defaults: add an i32 flag param before the f64 param
3. Remove the sNaN sentinel, i64.reinterpret ops, and pushParamSentinel
4. Function signatures change for expression-default params (breaking for external callers)

## Acceptance criteria

- No magic NaN values
- `f(NaN)` and `f()` correctly distinguished
- Constant defaults inlined at call site
- Expression defaults use boolean flag
