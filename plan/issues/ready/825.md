...
1. **Direct eval + `arguments` remains a major residual source**. Many of the
   ...
   the eval-scope / parameter-environment problems already described in #858.
2. **Destructuring is no longer the whole story**. The remaining bucket mixes
   ...
   umbrella, while #858 now covers a larger share of the highest-volume residual
   subpattern.

## Test Results

**Fix**: Added host-import fallback in `destructureParamArray` for externref array destructuring.
When no WasmGC vec type matches via `ref.test`, uses `__extern_length` + `__extern_get_idx`
to build a `vec_externref` from any JS array-like object.

**Sampled results** (100 dstr tests across class/dstr, expressions/class/dstr, object/dstr, for-of/dstr):
- PASS: 45/100
- NULL_DEREF: 3/100 (down from ~60% before fix)
- CE: 14/100
- OTHER_FAIL: 38/100

**Regression check** (30 non-dstr tests): 0 null derefs introduced, 15/30 pass (pre-existing failures only).
