...
Our comparison table in the blog post has "No benchmarks yet" for ts2wasm and "No data" for several competitors. We cite third-party claims (Porffor: "10-30x faster than interpreter-bundling", QuickJS: "20-100x slower than V8") but have not independently verified any of them. We need our own numbers.
...
- Any benchmark that uses features ts2wasm doesn't support yet (start with what we can compile)
...
- This is also a good stress test for ts2wasm's conformance: if a benchmark fails to compile, that's a data point too.
