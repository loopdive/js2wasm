...
Implement `wasi:http/incoming-handler` interface so ts2wasm output can serve HTTP requests on edge platforms (Fastly Compute, Fermyon Spin, Cloudflare Workers via WASI).
