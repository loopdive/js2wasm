...
- **Module graph** — prettier 3.x is ESM with ~500 source files; pre-bundling via #1041 is required. Otherwise every cross-file reference becomes `declare const X: any`.
...
1. **#1041 pre-bundle scaffold** — required input path
2. **Large-switch codegen scaling** — profile AND verify it compiles and runs for the ~200-case estree printer. If it doesn't, a jump-table pass in codegen is the fix. Filing as part of the stress-test output, not yet a separate issue.
3. **`String.prototype.matchAll` + RegExp edge-case coverage audit** — verify before Tier 4 runs; any gap will show as a self-format diff.
...
**Recommendation:** attempt prettier first, even before lodash. Start with Tier 1 + Tier 2 (doc printer) on a pre-bundled input after #1041 lands. Expect the self-format diff to surface 10-30 small correctness bugs — each becomes a follow-up issue. This is the highest-signal, lowest-design-overhead stress test of the four.
