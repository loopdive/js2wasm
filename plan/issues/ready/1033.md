...
- **`DOM_HOST_GLOBALS` not defined.** `document`, `window`, `HTMLElement`, `Event`, `Node`, `requestAnimationFrame`, `queueMicrotask`, etc. are not registered as extern classes. Every `document.createElement(...)` call hits `preprocessImports` (src/import-resolver.ts:23) as an unresolved ambient global. Needs the same compile-time routing mechanism as Node builtins in #1032 but scoped to DOM globals. Part of this issue's own scope.
...
### Projected readiness (JS-host mode, pre-bundled, production build preferred)
...
3. **#1041 pre-bundle scaffold** — React's module graph has ~500 files; pre-bundling is the only tractable input path.
