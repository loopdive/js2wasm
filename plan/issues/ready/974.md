...
- `scripts/generate-feature-examples.ts`: defines 47 features with JS snippets,
  compiles each with `compile(src, { emitWat: true })`, extracts the most
  illustrative WAT function (tier 1: named non-utility, tier 2: closures/helpers,
  tier 3: `$__module_init`), truncates at 15 lines. 37 compile, 10 skipped
  (unsupported features like eval, Proxy, SharedArrayBuffer, etc.)
- `public/feature-examples.json`: generated static asset (committed), loaded
  by index.html at runtime via `fetch("./feature-examples.json")`
- `index.html`: replaced 950+ lines of hand-maintained HTML with a 100-line
  client-side render script that uses `textContent` for automatic HTML escaping
- WAT extraction: filters import stubs (`*_import`), internal helpers
  (`$__vec_*`, `$__sget_*`, `$__ng_*`), prefers operation functions over
  constructors (`_new` suffix), falls back to `$__module_init`

## Test Results

- 37/47 features compile successfully (0 unexpected failures)
- 10 features skipped (noCompile=true for eval, with, Proxy, SharedArrayBuffer,
  WeakRef, Dynamic import, Promise.then, Top-level await, Temporal,
  Object.defineProperty)
- Pre-existing equivalence test failures unchanged (no regression)
