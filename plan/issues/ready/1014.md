...
- **6/6 unit tests pass** (tests/issue-1014.test.ts)
- **141/241 async generator expression tests pass** (was ~0 before — .next() always returned plain object)
...
- Remaining failures (53 CE, 26 FAIL, 21 RUNTIME ERR) are pre-existing: early-error syntax tests, default parameter handling, prototype chain issues — all unrelated to the `.then()` fix
