...
function generateEnvImportLine(name: string, mod: WasmModule): string {
  ...
  if (name === "__create_async_generator")
    return `${name}: (buf, pendingThrow) => { let i = 0; function mkR(v, d) { const p = { value: v, done: d }; return { value: v, done: d, then(res, rej) { return Promise.resolve(p).then(res, rej); } }; } return { next() { if (i < buf.length) return mkR(buf[i++], false); if (pendingThrow !== null && pendingThrow !== undefined) { const e = pendingThrow; pendingThrow = null; return Promise.reject(e); } return mkR(undefined, true); }, return(v) { i = buf.length; return mkR(v, true); }, throw(e) { i = buf.length; return Promise.reject(e); }, [Symbol.asyncIterator]() { return this; } }; }`;
