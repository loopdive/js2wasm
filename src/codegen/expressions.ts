...
/** Emit the arithmetic operation for a compound assignment */
...
/** obj.prop += value — compound assignment on struct property */
function compilePropertyCompoundAssignment(
  ...
  const typeName = resolveStructName(ctx, objType);
  if (!typeName) {
    ctx.errors.push({ message: `Cannot resolve struct for compound assignment on property: ${propName}`, line: getLine(target), column: getCol(target) });
    return null;
  }
  const structTypeIdx = ctx.structMap.get(typeName);
  const fields = ctx.structFields.get(typeName);
  if (structTypeIdx === undefined || !fields) {
    ctx.errors.push({ message: `Unknown struct type for compound assignment: ${typeName}`, line: getLine(target), column: getCol(target) });
    return null;
  }
  const fieldIdx = fields.findIndex((f) => f.name === propName);
  if (fieldIdx === -1) {
    ctx.errors.push({ message: `Unknown field for compound assignment: ${propName}`, line: getLine(target), column: getCol(target) });
    return null;
  }
  ...
  const objLocal = allocLocal(fctx, `__cmpd_obj_${fctx.locals.length}`, objResult);
  fctx.body.push({ op: "local.set", index: objLocal });
  ...
  // Compile RHS
  const rhsType = compileExpression(ctx, fctx, value, { kind: "f64" });
  ...
  // Save result
  const resultLocal = allocLocal(fctx, `__cmpd_res_${fctx.locals.length}`, { kind: "f64" });
...
/** arr[i] += value — compound assignment on element access */
function compileElementCompoundAssignment(
  ...
  // String-literal bracket access on struct: obj["prop"] += value
  if (typeDef?.kind === "struct" && ts.isStringLiteral(target.argumentExpression)) {
    const propName = target.argumentExpression.text;
    const fieldIdx = typeDef.fields.findIndex((f: { name: string }) => f.name === propName);
    if (fieldIdx !== -1) {
      const objLocal = allocLocal(fctx, `__cmpd_obj_${fctx.locals.length}`, arrType);
      fctx.body.push({ op: "local.set", index: objLocal });
      fctx.body.push({ op: "local.get", index: objLocal });
      fctx.body.push({ op: "struct.get", typeIdx, fieldIdx });
      const fieldType = typeDef.fields[fieldIdx]!.type;
  ...
  if (isVecStruct) {
    ...
    const vecLocal = allocLocal(fctx, `__cmpd_vec_${fctx.locals.length}`, arrType);
    fctx.body.push({ op: "local.set", index: vecLocal });
    const idxResult = compileExpression(ctx, fctx, target.argumentExpression, { kind: "f64" });
    if (!idxResult) return null;
    fctx.body.push({ op: "i32.trunc_f64_s" });
    const idxLocal = allocLocal(fctx, `__cmpd_idx_${fctx.locals.length}`, { kind: "i32" });
    fctx.body.push({ op: "local.set", index: idxLocal });
  ...
  ctx.errors.push({ message: "Unsupported compound assignment element access target", line: getLine(target), column: getCol(target) });
  return null;
}
  ...
  switch (expr.operator) {
    ...
    case ts.SyntaxKind.MinusMinusToken: {
      ...
      if (ts.isPropertyAccessExpression(expr.operand)) {
        return compilePrefixIncrementProperty(ctx, fctx, expr.operand, isIncrement);
      }

      // ++arr[i] / --arr[i] (element access target)
      if (ts.isElementAccessExpression(expr.operand)) {
        return compilePrefixIncrementElement(ctx, fctx, expr.operand, isIncrement);
      }
