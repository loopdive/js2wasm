...
function compileObjectDestructuring(
  ...
  if (resultType.kind === "ref" || resultType.kind === "ref_null") {
    ...
    const resolvedName = ctx.typeIdxToStructName.get(actualTypeIdx);
    if (resolvedName) {
      typeName = resolvedName;
      structTypeIdx = actualTypeIdx;
      fields = ctx.structFields.get(resolvedName);
    }
  }
  ...
  emitNullGuard(fctx, tmpLocal, resultType.kind === "ref_null", () => {
  ...
  for (const element of pattern.elements) {
    ...
    if (ts.isObjectBindingPattern(element.name) || ts.isArrayBindingPattern(element.name)) {
      ...
      if (ts.isObjectBindingPattern(element.name) && (nFieldType.kind === "ref" || nFieldType.kind === "ref_null")) {
        ...
        if (nestedFields) {
          emitNullGuard(fctx, nestedTmp, nFieldType.kind === "ref_null", () => {
...
function compileArrayDestructuring(
  ...
  if (isTupleStruct) {
    ...
    emitNullGuard(fctx, tmpLocal, isNullableArr, () => {
      for (let i = 0; i < pattern.elements.length; i++) {
        const element = pattern.elements[i]!;
        if (ts.isOmittedExpression(element)) continue;
...
function compileForOfDestructuring(
  ...
  if (ts.isObjectBindingPattern(pattern)) {
    ...
    emitNullGuard(fctx, elemLocal, elemType.kind === "ref_null", () => {
