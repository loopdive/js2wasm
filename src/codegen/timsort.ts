...
function BR(d: number): Instr { return { op: "br", depth: d }; }
function BR_IF(d: number): Instr { return { op: "br_if", depth: d }; }
function IF(then: Instr[], els?: Instr[]): Instr {
  const r: any = { op: "if", blockType: { kind: "empty" }, then };
  if (els) r.else = els;
  return r;
}
function CALL(idx: number): Instr { return { op: "call", funcIdx: idx }; }
...
function emitInsertionSort(
  ...
  const DATA = 0, LO = 1, HI = 2, II = 3, JJ = 4, KEY = 5;
  ...
  const body: Instr[] = [
    ...
    BLOCK([
      LOOP([
        // if i >= hi: break
        L(II), L(HI), GE_S, BR_IF(1),

        // key = data[i]
        L(DATA), L(II), AG(arrTypeIdx), LS(KEY),
        ...
        BLOCK([
          LOOP([
            L(JJ), L(LO), LT_S, BR_IF(1),
            L(DATA), L(JJ), AG(arrTypeIdx), L(KEY), gtOp, EQZ, BR_IF(1),
            // data[j+1] = data[j]
            L(DATA), L(JJ), I(1), ADD, L(DATA), L(JJ), AG(arrTypeIdx), AS(arrTypeIdx),
            // j--
            L(JJ), I(1), SUB, LS(JJ),
            BR(0),
          ]),
        ...
        // data[j+1] = key
        L(DATA), L(JJ), I(1), ADD, L(KEY), AS(arrTypeIdx),
...
function emitMerge(
  ...
  const DATA = 0, TMP = 1, LO = 2, MID = 3, HI = 4;
  const LEFT = 5, II = 6, JJ = 7, KK = 8;

  const body: Instr[] = [
    // leftLen = mid - lo
    L(MID), L(LO), SUB, LS(LEFT),
    // if leftLen <= 0: return
    L(LEFT), I(0), LE_S, IF([RET]),

    // Copy left half to tmp[0..leftLen)
    L(TMP), I(0), L(DATA), L(LO), L(LEFT), AC(arrTypeIdx, arrTypeIdx),
    ...
    BLOCK([
      LOOP([
        L(II), L(LEFT), GE_S, BR_IF(1),  // left exhausted
        L(JJ), L(HI), GE_S, BR_IF(1),    // right exhausted

        // if tmp[i] <= data[j]: take from left, else from right
        L(TMP), L(II), AG(arrTypeIdx),
        L(DATA), L(JJ), AG(arrTypeIdx),
        leOp,
        IF(
          [
            L(DATA), L(KK), L(TMP), L(II), AG(arrTypeIdx), AS(arrTypeIdx),
            L(II), I(1), ADD, LS(II),
          ],
          [
            L(DATA), L(KK), L(DATA), L(JJ), AG(arrTypeIdx), AS(arrTypeIdx),
            L(JJ), I(1), ADD, LS(JJ),
          ],
        ),
    ...
    L(II), L(LEFT), LT_S,
    IF([
      L(DATA), L(KK), L(TMP), L(II), L(LEFT), L(II), SUB, AC(arrTypeIdx, arrTypeIdx),
    ]),
  ];
...
function emitMergeRun(
  ...
  const DATA = 0, TMP = 1, SBASE = 2, SLEN = 3, SSIZE = 4, IDX = 5;
  const BASE1 = 6, LEN1 = 7, LEN2 = 8;

  const body: Instr[] = [
    // base1 = sBase[idx]
    L(SBASE), L(IDX), AG(i32ArrTypeIdx), LS(BASE1),
    // len1 = sLen[idx]
    L(SLEN), L(IDX), AG(i32ArrTypeIdx), LS(LEN1),
    // len2 = sLen[idx + 1]
    L(SLEN), L(IDX), I(1), ADD, AG(i32ArrTypeIdx), LS(LEN2),

    // call __merge(data, tmp, base1, base1+len1, base1+len1+len2)
    L(DATA), L(TMP),
    L(BASE1),
    L(BASE1), L(LEN1), ADD,
    L(BASE1), L(LEN1), ADD, L(LEN2), ADD,
    CALL(mergeFuncIdx),

    // sLen[idx] = len1 + len2
    L(SLEN), L(IDX), L(LEN1), L(LEN2), ADD, AS(i32ArrTypeIdx),

    // Shift stack entries: copy from idx+2.. to idx+1..
    // length = stackSize - idx - 2 (always >= 0)
    L(SBASE), L(IDX), I(1), ADD,
    L(SBASE), L(IDX), I(2), ADD,
    L(SSIZE), L(IDX), SUB, I(2), SUB,
    AC(i32ArrTypeIdx, i32ArrTypeIdx),

    L(SLEN), L(IDX), I(1), ADD,
    L(SLEN), L(IDX), I(2), ADD,
    L(SSIZE), L(IDX), SUB, I(2), SUB,
    AC(i32ArrTypeIdx, i32ArrTypeIdx),
...
function emitTimsortMain(
  ...
  const VEC = 0;
  // Locals 1..19
  const DATA = 1, LEN = 2, TMP = 3, MIN_RUN = 4;
  const NMR = 5, RMR = 6;
  const LO = 7, RUN_END = 8, RUN_LEN = 9, FORCE = 10;
  const SB = 11, SL = 12, SS = 13;
  const SN = 14, MIDX = 15;
  const I_REV = 16, J_REV = 17, T_SWAP = 18, SHOULD = 19;
  ...
  const sec1: Instr[] = [
    L(VEC), { op: "struct.get", typeIdx: vecTypeIdx, fieldIdx: 1 }, LS(DATA),
    L(VEC), { op: "struct.get", typeIdx: vecTypeIdx, fieldIdx: 0 }, LS(LEN),
    // if len < 2: return
    L(LEN), I(2), LT_S, IF([RET]),
  ];
  ...
  const sec2: Instr[] = [
    L(LEN), I(64), LT_S,
    IF([
      L(DATA), I(0), L(LEN), CALL(isortIdx),
      RET,
    ]),
  ];
  ...
  const sec3: Instr[] = [
    ...
    BLOCK([
      LOOP([
        L(NMR), I(64), LT_S, BR_IF(1),
        // rmr |= (nmr & 1)
        L(RMR), L(NMR), I(1), I_AND, I_OR, LS(RMR),
        // nmr >>= 1
        L(NMR), I(1), SHR_U, LS(NMR),
        BR(0),
      ]),
    ]),
    L(NMR), L(RMR), ADD, LS(MIN_RUN),
  ];

  // -- Section 4: allocate buffers --
  const sec4: Instr[] = [
    L(LEN), AND(arrTypeIdx), LS(TMP),
    I(85), AND(i32ArrTypeIdx), LS(SB),
    I(85), AND(i32ArrTypeIdx), LS(SL),
    I(0), LS(SS),
    I(0), LS(LO),
  ];
  ...
  const detectRun: Instr[] = [
    // runEnd = lo + 1
    L(LO), I(1), ADD, LS(RUN_END),

    L(RUN_END), L(LEN), GE_S,
    IF(
      // Single element remaining
      [I(1), LS(RUN_LEN)],
      [
        // Check if descending: data[lo+1] < data[lo]
        L(DATA), L(RUN_END), AG(arrTypeIdx),
        L(DATA), L(LO), AG(arrTypeIdx),
        ltOp,
        IF(
          // Descending run
          [
            L(RUN_END), I(1), ADD, LS(RUN_END),
            BLOCK([
              LOOP([
                L(RUN_END), L(LEN), GE_S, BR_IF(1),
                // if data[runEnd] >= data[runEnd-1]: stop
                L(DATA), L(RUN_END), AG(arrTypeIdx),
                L(DATA), L(RUN_END), I(1), SUB, AG(arrTypeIdx),
                ltOp, EQZ, BR_IF(1),
                L(RUN_END), I(1), ADD, LS(RUN_END),
                BR(0),
              ]),
            ...
            // Reverse [lo, runEnd)
            L(LO), LS(I_REV),
            L(RUN_END), I(1), SUB, LS(J_REV),
            BLOCK([
              LOOP([
                L(I_REV), L(J_REV), GE_S, BR_IF(1),
                // swap data[iRev] <-> data[jRev]
                L(DATA), L(I_REV), AG(arrTypeIdx), LS(T_SWAP),
                L(DATA), L(I_REV), L(DATA), L(J_REV), AG(arrTypeIdx), AS(arrTypeIdx),
                L(DATA), L(J_REV), L(T_SWAP), AS(arrTypeIdx),
                L(I_REV), I(1), ADD, LS(I_REV),
                L(J_REV), I(1), SUB, LS(J_REV),
                BR(0),
              ]),
            ]),
          ],
  ...
  const extendRun: Instr[] = [
    ...
    L(MIN_RUN), L(LEN), L(LO), SUB,
    // select: if minRun <= len-lo then minRun else len-lo
    L(MIN_RUN), L(LEN), L(LO), SUB, LE_S,
    { op: "select" },
    LS(FORCE),

    L(RUN_LEN), L(FORCE), LT_S,
    IF([
      L(DATA), L(LO), L(LO), L(FORCE), ADD, CALL(isortIdx),
      L(FORCE), LS(RUN_LEN),
    ]),
  ];
  ...
  const pushRun: Instr[] = [
    L(SB), L(SS), L(LO), AS(i32ArrTypeIdx),
    L(SL), L(SS), L(RUN_LEN), AS(i32ArrTypeIdx),
    L(SS), I(1), ADD, LS(SS),
  ];
  ...
  const mergeCollapse: Instr[] = [
    BLOCK([
      LOOP([
        L(SS), I(2), LT_S, BR_IF(1), // stackSize < 2: done

        L(SS), I(2), SUB, LS(SN),    // sn = stackSize - 2
        I(0), LS(SHOULD),
        L(SN), LS(MIDX),             // default mergeIdx = sn
        ...
        IF(
          ...
          [
            ...
            L(SL), L(SN), I(1), SUB, AG(i32ArrTypeIdx),  // runLen[sn-1]
            L(SL), L(SN), AG(i32ArrTypeIdx),              // runLen[sn]
            L(SL), L(SN), I(1), ADD, AG(i32ArrTypeIdx),   // runLen[sn+1]
            ...
            IF(
              [
                ...
                L(SL), L(SN), I(1), SUB, AG(i32ArrTypeIdx),
                L(SL), L(SN), I(1), ADD, AG(i32ArrTypeIdx),
                LT_S,
                IF([L(SN), I(1), SUB, LS(MIDX)]),
              ],
              // Invariant 1 OK, check invariant 2: runLen[sn] <= runLen[sn+1]
              [
                L(SL), L(SN), AG(i32ArrTypeIdx),
                L(SL), L(SN), I(1), ADD, AG(i32ArrTypeIdx),
                LE_S,
                IF([I(1), LS(SHOULD)]),
              ],
        ...
        L(SHOULD), EQZ, BR_IF(1), // no merge needed: done

        // call __merge_run(data, tmp, sBase, sLen, stackSize, mergeIdx)
        L(DATA), L(TMP), L(SB), L(SL), L(SS), L(MIDX),
        CALL(mergeRunIdx),
        LS(SS),
  ...
  // 5e: advance lo
  const advanceLo: Instr[] = [
    L(LO), L(RUN_LEN), ADD, LS(LO),
  ];
