...
## Important Rules
- **Never kill running tests without asking** — always ask the user before killing test262, vitest, or benchmark processes, even if the data appears stale
- **Always `cd /workspace &&`** before git commands on main — avoid operating on worktree branches accidentally
- **Serialize cherry-picks** — wait for all agents in a wave to finish, then batch cherry-pick
