...
- **PO writes**: `plan/issues/`, `plan/progress/`, `plan/dependency-graph.md`, `plan/issues/backlog/backlog.md`, `plan/graph-data.json`, `plan/issues-graph.html`, `plan/generate-graph.ts`
...
**How to apply:** When you need a code change (runner fix, skip filter, etc.), create an issue in `plan/issues/ready/` describing what needs to change. Don't edit the file yourself.
